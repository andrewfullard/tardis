...
class DirectBetaSobolevSolver(SobolevPopulationSolver):
    """Use the upper-level population directly as beta for damping tests."""
...
def test_sobolev_damps_a_growing_reversing_update() -> None:
    """A growing reversed update must halve the next Picard step."""
    ...
    np.testing.assert_allclose(
        population_solver.beta_values[:3], [0.2, 0.8, 0.4]
    )
    np.testing.assert_allclose(state.level_number_density.iloc[1], 0.5)
    np.testing.assert_allclose(beta_sobolev, 0.5)


def test_sobolev_damps_slowly_decaying_alternating_updates() -> None:
    """Alternating updates must be damped before iteration exhaustion."""

    class AlternatingPopulationSolver(BetaPopulationSolver):
        """Return a fixed-point update with a slowly decaying alternating mode."""
        ...
        def solve(
            ...
            del electron_density_seed
            beta_value = float(beta_sobolev.iloc[0, 0])
            upper_population = 0.5 - 0.99 * (beta_value - 0.5)
            level_number_density = pd.DataFrame(
                [[1.0], [upper_population]],
                index=LEVEL_INDEX,
                columns=[0],
    ...
    initial_beta_sobolev = pd.DataFrame([[0.1]], index=LINE_INDEX, columns=[0])
    ...
    np.testing.assert_allclose(beta_sobolev, 0.5, rtol=1e-7)
    np.testing.assert_allclose(
        state.level_number_density.iloc[1], 0.5, rtol=1e-7
    )
