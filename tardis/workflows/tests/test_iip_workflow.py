...
def test_standard_initial_ion_ratio_matches_ion_populations(
    ...
    expected = (
        ...
        / plasma.ion_number_density.loc[(1, 0)]
    )
...
def test_standard_thermal_rates_match_iip_plasma_after_mc(
    ...
    """Compare every thermal process using one identical post-MC state."""
    ...
    for process, (heating, cooling) in standard_rates.items():
        # The legacy module uses Astropy's current constants while the
        # standard plasma uses TARDIS's pinned astropy13 constants. Bound-free
        # rates also differ slightly in integration order.
        relative_tolerance = {
            "bound_free": 2e-6,
    ...
    for _iteration in range(2):
        ...
        assert np.all(
            np.isfinite(
                opacity_states[
                    "macro_atom_state"
                ].transition_probabilities.to_numpy()
            )
        )
